...
const registerUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (!username || !email || !password) {
      return res
        .status(404)
        .json({ message: "username email or password something missing" });
    }
    ...
    const user = await User.findById(userCreation._id).select(
    ...
    if (user) {
      ...
      return res
        .status(200)
        .json({ message: "User registerd Failed", success: false, data: null });
    }
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!username || !password) {
    return res
      .status(404)
      .json({ message: "Username or password is missing !", success: false });
  }
  ...
  if (!user) {
    return res.status(404).json({
      message: "User not Found , Please Registered yourself !",
      success: false,
    });
  }
  ...
  if (!generateAccessToken) {
    return res
      .status(302)
      .json({ message: "Failed to generate Access Token", success: false });
  } else {
  ...
  if (!generateRefreshToken) {
    return res
      .status(302)
      .json({ message: "Failed to generate Refresh Token", success: false });
  } else {
    await user.save();
  ...
  return res
    .status(200)
    .cookie("accToken", generateAccessToken, options)
...
const refreshAccessToken = async (req, res) => {
  try {
    ...
    if (!verifyRefToken) {
      return res
        .status(404)
        .json({ message: "Token Expire !", success: false, status: undefined });
    }
    ...
    return res.status(200).cookie("accToken", accessToken, option).json({
      message: "AccessToken Generation successfully !",
      status: "Ok",
      success: true,
...
const getUsers = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "fetched all users successfully ", data: findallUser });
});
const getUser = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({
    message: "User's profile fetch successfully ",
    data: findUser,
    customMessage: req.customMessage,
  });
});
